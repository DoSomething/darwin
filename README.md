...
All of our code is open source and on GitHub. We organize stories and plan sprints with Pivotal Tracker, and maintain our roadmaps \(currently\) in Google Spreadsheets . Individual coding tasks are tracked as Pivotal cards. We use pull requests to review and merge changes. Every PR is peer-reviewed. Please see here for an overview of our [Teams, Services, and Environments](http://docs.dosomething.org/engineering/overview). We communicate on [Slack](slack.md).
...
More information on the roles and responsibilities can be viewed either as a grid [here](https://docs.google.com/spreadsheets/d/1KcQWgX2YPk_fOxWEYHk5lxojYtOBrvvACmcTFh0HJMo/edit?usp=sharing) or on the different position pages linked to above.
